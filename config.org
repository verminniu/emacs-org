...
#+begin_src emacs-lisp :tangle yes
  (use-package system-packages
    :ensure t
    :config
    ;; 1. 定义 zypper 规则 (如果插件本身没包含的话)
    (unless (assoc 'zypper system-packages-supported-package-managers)
      (add-to-list 'system-packages-supported-package-managers
                   '(zypper . ((default-sudo . t)
    ...
    (cond
     ...
     ((executable-find "zypper")
      (setq system-packages-package-manager 'zypper))
...
** 系统包提醒（只打印，不安装）
#+begin_src emacs-lisp :tangle yes
  (defmacro my/ensure-pkg (pkg-name &optional system-pkg-name)
    "检查系统命令 PKG-NAME 是否存在，如果不存在则根据系统给出安装建议。
  如果系统包名与命令名不同，可传入 SYSTEM-PKG-NAME。"
    `(let ((exec-name ,pkg-name)
           (install-name (or ,system-pkg-name ,pkg-name)))
       (unless (executable-find exec-name)
         (let ((install-prompt
                (cond
                 ((executable-find "zypper") (format "sudo zypper in %s" install-name))
                 ((executable-find "brew")   (format "brew install %s" install-name))
                 ((executable-find "pacman") (format "sudo pacman -S %s" install-name))
                 ((executable-find "apt")    (format "sudo apt install %s" install-name))
                 (t (format "请手动安装包: %s" install-name)))))
           (display-warning 'SystemDependency
                            (format "--- [依赖缺失] --- 执行以下命令安装 %s:\n    %s" exec-name install-prompt)
                            :warning)))))
#+end_src
* Start Up
** Better defaults
...
#+BEGIN_SRC emacs-lisp :tangle yes
    ...
    ;; 强制 Emacs 在加载时如果发现源码更新，则忽略旧的字节码
...
#+begin_src emacs-lisp :tangle yes
  (use-package awesome-tray
    ...
    ;; --- 自定义 Git 显示格式 ---
    ;; 1. 设置你想要显示的信息模块 https://github.com/manateelazycat/awesome-tray
    (setq awesome-tray-active-modules '("git" "mode-name"  "location"))

    ;; 2. 适配 Nerd Icons (手动指定图标)
    ;; awesome-tray 默认使用文本，你可以通过自定义 face 或简单的字符串注入图标
    (setq awesome-tray-separator "  ") ; 模块之间的间距
    )
#+end_src


#+begin_src emacs-lisp :tangle no
  ;; 主动安装/加载最新版 compat -- moody 依赖31版本
  (use-package compat
    :ensure t)
...
#+begin_src emacs-lisp :tangle yes
  ...
  (use-package project
    ...
    :config
    ;; 让 project.el 也能识别一些非 Git 项目（比如有 .project 文件的目录）
    (setq project-vc-extra-files '(".project" "go.mod" "package.json"))
    ;; --- 核心：换掉 C-c p s ---
    ;; Projectile 的 s 通常是搜索，我们将它绑定到 consult-ripgrep
    (define-key project-prefix-map (kbd "s") #'consult-ripgrep)

    ;; 模仿 Projectile，将 C-c p a 设为切换头文件/源文件
    (define-key project-prefix-map (kbd "a") #'ff-find-other-file)
    ;; --- 对齐其他 Projectile 习惯 ---
    (define-key project-prefix-map (kbd "p") #'project-switch-project)     ;; 对应 C-c p p
    (define-key project-prefix-map (kbd "f") #'project-find-file)          ;; 对应 C-c p f
    (define-key project-prefix-map (kbd "b") #'consult-project-buffer)    ;; 对应 C-c p b
    (define-key project-prefix-map (kbd "d") #'project-dired)             ;; 对应 C-c p d
    (define-key project-prefix-map (kbd "k") #'project-kill-buffers)      ;; 对应 C-c p k
    (define-key project-prefix-map (kbd "v") #'magit-project-status)      ;; 对应 C-c p v (Magit)

    ;; 如果你还想要原先的 shell，可以把它挪到一个不常用的键位
    (define-key project-prefix-map (kbd "!") #'project-shell)
    (setq project-switch-commands
    '((project-find-file "Find file" "f")
      (consult-ripgrep "Ripgrep" "s") ;; 对应上面的 C-c p s
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (use-package lsp-mode
    ...
    (setq lsp-log-io nil)
    (setq lsp-enable-file-watchers nil)  ;; 防止大项目卡死

    (setq lsp-auto-guess-root t)       ; Detect project root
    ;; 这一步很重要：确保 LSP 寻找项目根目录的逻辑与 project.el 一致
    (setq lsp-project-relative-root-functions '(lsp--project-root-project-el))
    ;; (setq lsp-keymap-prefix "C-c l")
    ...
    (setq lsp-clients-clangd-args
          '("--header-insertion=iwyu"         ; 开启自动插入
            "--header-insertion-decorators"
            "--background-index"              ; 开启后台索引
            "--clang-tidy"                    ; 开启静态检查
            "--completion-style=detailed"     ; 提供详细补全信息
            "--pch-storage=memory"            ; PCH 存储在内存中，大幅提升速度
            "-j=12"                           ; 并行线程数，建议设为核心数的 1.5 倍
            "--limit-results=50"              ; 限制补全结果数量，防止渲染卡顿
            "--all-scopes-completion"         ; 全局范围补全
            "--header-insertion-decorators")) ; 装饰符提示
    ...
    ;; 开启 LSP 原生实用 UI 功能
    (setq lsp-headerline-breadcrumb-enable t) ;; 顶部路径面包屑
    (setq lsp-modeline-code-actions-enable t) ;; 底部 code action 提示
    (setq lsp-lens-enable t)                  ;; 代码透镜 (显示引用次数)

    ;; 修复 lsp-mode 默认匹配逻辑与 Orderless 的冲突
  ...
  (use-package lsp-ui
    ...
    ;; 悬浮文档 (Hover)
    (lsp-ui-doc-enable t)
    (lsp-ui-doc-position 'at-point)      ;; 悬浮窗紧贴光标
    (lsp-ui-doc-show-with-cursor t)      ;; 光标停留自动显示
    (lsp-ui-doc-show-with-mouse nil)     ;; 关闭鼠标悬停显示，防止乱弹
    (lsp-ui-doc-max-width 80)
    (lsp-ui-doc-max-height 20)
    (lsp-ui-doc-border (face-foreground 'default)) ;; 加个漂亮的边框

    ;; 右侧信息栏 (Sideline)
    (lsp-ui-sideline-enable t)
    (lsp-ui-sideline-show-diagnostics t) ;; 右侧显示 Error/Warning
    (lsp-ui-sideline-show-code-actions t)
    (lsp-ui-sideline-show-hover nil)     ;; 关掉 sideline 的 hover，避免和 doc 冲突
...
#+end_src
** apheleia
这个好像比formatall好使，先替换试试
#+begin_src emacs-lisp :tangle yes
  (use-package apheleia
    :ensure t
    :config
    ;; 1. 定义自定义格式化器 (Custom Formatters)
    ;; Apheleia 默认已经内置了 black, clang-format 等，但我们可以按你的需求覆盖它们
    ;; 定制 pg_format (SQL)
    (setf (alist-get 'pg_format apheleia-formatters)
          '("pg_format" "-"))

    ;; 定制 clang-format (使用 Google 风格)
    (setf (alist-get 'g-clang-format apheleia-formatters)
          '("clang-format" "-style=google" "-assume-filename"
            (or (buffer-file-name)
                (cl-case major-mode
                  (c-mode "test.c")
                  (c++-mode "test.cpp")
    ...
    ;; 定制 black (Python)
    (setf (alist-get 'my-black apheleia-formatters)
          '("black" "-q" "--experimental-string-processing"
            (when (and (buffer-file-name)
                       (string-suffix-p ".pyi" (buffer-file-name)))
              "--pyi")
            "-"))

    ;; 2. 映射 Major Mode 到格式化器 (Mode Alist)
    (setq apheleia-mode-alist
          (append '(;; 使用自定义的格式化器
                    (sql-mode . pg_format)
                    (c-mode   . g-clang-format)
                    (c++-mode . g-clang-format)
                    (python-mode . my-black)
                    ;; 使用内置默认的格式化器
...
#+begin_src emacs-lisp :tangle no
  (use-package format-all
    ...
    ;;  (black . black))
    :init
    (my/ensure-pkg "pg_format" "pgformatter")
    (my/ensure-pkg "black" "black")
    :config
...
#+end_src
** Python
Lsp-mode/eglot will start py on python mode
...
#+begin_src emacs-lisp :tangle yes
  ;; ==========================================
  ;; 2. Minibuffer 补全全家桶 (完美替代 Ivy + Swiper)
  ;; ==========================================
  ;; Vertico: 极简极速的垂直补全列表
  (use-package vertico
    :ensure t
    :init (vertico-mode)
    :custom
    ;; 1. 设置固定显示 10 行结果（你可以根据喜好改成 15 或 20）
    (vertico-count 10)
    ;; 2. 关键：将缩放设置为 nil，关闭自动扩展/收缩
    (vertico-resize nil)
    (vertico-cycle t))          ;; 允许循环滚动

  ;; Orderless: 神级模糊匹配 (支持空格分隔多关键词)
  (use-package orderless
    :ensure t
    :custom
  ...
  ;; Marginalia: 列表右侧显示详细注解 (类似 ivy-rich)
  (use-package marginalia
    :ensure t
    :init (marginalia-mode))

  ;; 2. 现代补全图标增强 (完美替代 nerd-icons-ivy-rich)
  (use-package nerd-icons-completion
    :ensure t
    :after (vertico marginalia) ;; 确保在 Vertico 和 Marginalia 之后加载
    :config
    (nerd-icons-completion-mode))

  ;; Consult: 高级搜索与跳转 (完美替代 Swiper 和 Counsel)
  (use-package consult
    :ensure t
    :custom
    (consult-preview-key nil)               ;; 关闭预览
    :bind (("C-s" . consult-line)           ;; 替代 swiper: 当前文件搜索
           ("C-M-s" . consult-ripgrep)      ;; 替代 counsel-rg: 全局搜代码
           ("C-x b" . consult-buffer)       ;; 替代 counsel-switch-buffer
           ("M-y" . consult-yank-pop)       ;; 替代 counsel-yank-pop: 剪贴板历史
           ("M-g g" . consult-goto-line)    ;; 极速跳行
           ("C-x C-r" . consult-recent-file)  ;; 最近文件
           ("C-c n" . consult-imenu)
           ("M-g o" . consult-outline)
           ("M-g i" . consult-imenu)))      ;; 替代 counsel-imenu: 大纲跳转

  ;; ==========================================
  ;; 3. 代码内补全全家桶 (完美替代 Company)
  ;; ==========================================
  ;; Corfu: 现代化的弹窗补全前端
  (use-package corfu
    :ensure t
    ...
    :custom
    (corfu-auto t)                  ;; 自动弹出补全
    (corfu-auto-delay 0.1)          ;; 极速响应
    (corfu-auto-prefix 1)           ;; 输入 1 个字符就开始提示
    (corfu-cycle t)                 ;; 允许循环选择
    (corfu-quit-at-boundary 'separator)
    (corfu-echo-documentation t)    ;; 在 echo area 显示文档摘要
    :bind (:map corfu-map
                ("TAB" . corfu-next)
                ([tab] . corfu-next)
  ...
  ;; Cape: 补全后端扩展 (替代 company-backends)
  (use-package cape
    :ensure t
    :init
    ;; 基础非 LSP 补全源
    (add-to-list 'completion-at-point-functions #'cape-file)    ;; 路径补全
    (add-to-list 'completion-at-point-functions #'cape-dabbrev) ;; 当前 Buffer 单词补全
    (add-to-list 'completion-at-point-functions #'cape-keyword))

  ;; Kind-icon: 类似 VS Code 的左侧彩色类型图标 (替代 company-box)
  ...
  (use-package embark
    :ensure t
    :bind
    (("C-;" . embark-act)         ;; 现代版的“右键菜单”
     ("C-c C-o" . embark-export)) ;; 在全局也可以使用导出
    :config
    ;; 导出后自动关闭 Minibuffer
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (use-package recentf
    ...
    (recentf-mode 1)
    :custom
    ;; 核心：不要让 recentf 去碰远程文件（TRAMP）
    (recentf-keep '(file-remote-p reverse nil))
    ;; 性能优化：关闭自动清理，改为手动或定时（防止每次打开文件都扫描磁盘）
    (recentf-auto-cleanup 'never)
    (recentf-max-saved-items 200)     ;; 增加记录数量，建议 200-500
    (recentf-max-menu-items 15)
    ;; 过滤掉不需要记录的路径（比如各种插件的自动保存、缓存）
    (recentf-exclude
     '("\\.?cache"
       "thumbnails"
       "gio-.*"
...
#+begin_src emacs-lisp :tangle yes
  (use-package difftastic
    ...
    :init
    (my/ensure-pkg "difft" "difftastic")
...
#+begin_src emacs-lisp :tangle yes
  (use-package pomo-cat
    :ensure (:host github :repo "kn66/pomo-cat.el"))
#+end_src
* Org-mode
** Org Common Setting
#+begin_src emacs-lisp :tangle yes
...
2. install org-mind-map
   #+begin_src emacs-lisp :tangle no
     ...
     (use-package org-mind-map
       :init
       (require 'ox-org)
       ;; (my/ensure-pkg "gvgen" "graphviz")
       ;; 只在mac下执行，linux大部分是tui，无意义
...
#+begin_src emacs-lisp :tangle no
  ...
  (use-package org-download
    :after org
    ;; 这个包只有mac下有，其他系统不设置，linux下为xclip，暂时未设置，先不考虑
    :if (eq system-type 'darwin)
    :ensure-system-package pngpaste
    ;; :init
    ;; (my/ensure-pkg "pngpaste" "pngpaste")
    :config
...
#+begin_src emacs-lisp :tangle yes
  (use-package elfeed
    ...
    ;; 1. 基础路径、过滤器与界面布局优化
    ;; ==========================================
    (setq elfeed-db-directory (expand-file-name "elfeed/db" user-emacs-directory)
          elfeed-search-filter "@2-months-ago +unread" ; 默认显示两个月内未读
          elfeed-search-title-max-width 90)

    ;; ==========================================
    ;; 2. 完整的订阅列表（含你提供及追加的所有国内外源）
    ;; ==========================================
    (setq elfeed-feeds
          '(
            ;; ===== 👤 你的专属独立博客列表 =====
            ("http://0x100.club" blogs)
            ("https://isamert.net/feeds.html" blogs emacs)
            ("http://pragmaticemacs.com/feed/" blogs emacs)
            ("https://blog.tecosaur.com/tmio/rss.xml" blogs emacs orgmode)
            ("https://nullprogram.com/feed/" blogs tech c linux)
            ("https://kitchingroup.cheme.cmu.edu/blog/" blogs emacs orgmode)
            ("https://protesilaos.com/feeds/" blogs emacs philosophy)
            ("http://weiwuhui.com/feed" blogs media)
            ("https://bzg.fr/index.xml" blogs emacs orgmode)
            ("https://alhassy.github.io/rss.xml" blogs emacs)
            ("http://blog.aaronbieber.com/feed.xml" blogs emacs)
            ("https://rgoswami.me/posts/index.xml" blogs tech science)
            ("https://archive.casouri.cat/note/rss.xml" blogs emacs)
            ("http://bnbeckwith.com/rss.xml" blogs)
            ("http://abovethecrowd.com" blogs finance)
            ("http://feeds.feedburner.com/xxddite" blogs tech)
            ("http://batsov.com/atom.xml" blogs emacs clojure)

            ;; ===== 🌐 英文硬核社区与技术媒体 =====
            ("https://news.ycombinator.com/rss" tech hackernews)
            ("https://hnrss.org/frontpage?comments=10" tech hackernews)
            ("https://lobste.rs/rss" tech lobsters)
            ("https://lwn.net/headlines/rss" tech linux open-source)

            ;; ===== ✍️ 国外顶级技术专家/黑客博客 =====
            ("https://danluu.com/atom.xml" blogs tech performance)
            ("https://jvns.ca/atom.xml" blogs tech networking linux)
            ("https://www.joelonsoftware.com/feed/" blogs tech management)
            ("https://martin创造.com/feed.xml" blogs tech architecture)

            ;; ===== 🤖 Reddit 优质技术子版块 =====
            ("https://www.reddit.com/r/emacs/.rss" reddit emacs)
            ("https://www.reddit.com/r/programming/.rss" reddit tech programming)
            ("https://www.reddit.com/r/linux/.rss" reddit tech linux)

            ;; ===== 🇨🇳 国内顶级大厂技术团队 =====
            ("https://tech.meituan.com/feed" tech china meituan-only)
            ("https://vlambda.com/feed" tech china)
            ("https://juejin.cn/rss" tech china)
            ("https://linux.cn/rss.xml" tech linux china)

            ;; ===== 📰 国内极客与数字生活媒体 =====
            ("https://sspai.com/feed" blogs china sspai-only)
            ("https://www.williamlong.info/catfeed.asp" blogs china tech)
            ("https://www.geekpark.net/rss" tech china media)
            ("https://36kr.com/feed" news china finance)

            ;; ===== 👤 国内顶尖个人极客/独立开发者 =====
            ("https://blog.tualatrix.com/feed.xml" blogs china emacs)
            ("https://coolshell.cn/feed" blogs tech china)
            ("https://macshuo.com/feed" blogs china tech)
            ("https://sspai.com/u/easy/posts/feed" blogs tech china)
            ("https://yinwang.org/atom.xml" blogs china philosophy yin-only)
            ))

    ;; ==========================================
    ;; 3. 视觉美化：根据不同来源/标签配置不同颜色
    ;; ==========================================
    (defface elfeed-color-emacs   '((t :foreground "#268bd2" :weight bold)) "Emacs 来源颜色")
    (defface elfeed-color-hn      '((t :foreground "#cb4b16" :weight bold)) "HackerNews 颜色")
    (defface elfeed-color-reddit  '((t :foreground "#ff4500")) "Reddit 颜色")
    (defface elfeed-color-china   '((t :foreground "#b8bb26")) "国内通用源颜色")
    (defface elfeed-color-blogs   '((t :foreground "#859900")) "普通独立博客颜色")

    ;; 专属定制网站颜色
    (defface elfeed-color-sspai   '((t :foreground "#da251d" :weight bold)) "少数派专属红")
    (defface elfeed-color-yin     '((t :foreground "#d33682" :slant italic)) "王垠专属斜体紫")
    (defface elfeed-color-meituan '((t :foreground "#00ced1" :weight bold)) "美团专属青")

    (setq elfeed-search-face-alist
          '((unread elfeed-search-unread-face) ; 未读默认加粗
            (sspai-only elfeed-color-sspai)
            (yin-only elfeed-color-yin)
            (meituan-only elfeed-color-meituan)
            (emacs elfeed-color-emacs)
            (hackernews elfeed-color-hn)
            (reddit elfeed-color-reddit)
            (china elfeed-color-china)
            (blogs elfeed-color-blogs)))

    ;; ==========================================
    ;; 4. 界面自定义渲染：标题前强制显示 [来源网站名称]
    ;; ==========================================
    (defun my-elfeed-search-print-entry (entry)
      "自定义 Elfeed 列表渲染：在标题前显示对齐的 [来源名称]"
      (let* ((date (elfeed-entry-date entry))
             ...
             ;; 获取该源的名称（若无则用URL替代）
             (feed-title (or (elfeed-feed-title feed) (elfeed-feed-url feed)))
             ;; 固定缩进宽度为 14 个字符，多出的截断，不够的补空格
             (feed-str (truncate-string-to-width (format "[%s]" feed-title) 14 0 ?\s))

             ;; 颜色匹配
             (title-face (elfeed-search--faces tags))
             (feed-face 'font-lock-keyword-face)
             (date-face 'font-lock-variable-name-face))

        (insert (propose-font-lock-face date-str date-face) " ")
        (insert (propose-font-lock-face feed-str feed-face) " ")
        (insert (propose-font-lock-face title title-face))
        (when tags
          (insert (concat " (" (propose-font-lock-face tags-str 'font-lock-comment-face) ")")))))

    (defun propose-font-lock-face (string face)
    ...
    ;; 视图过滤组合键 (连按 v 激活)
    (define-key elfeed-search-mode-map (kbd "v a") (lambda () (interactive) (elfeed-search-set-filter "@2-months-ago +unread")))
    (define-key elfeed-search-mode-map (kbd "v e") (lambda () (interactive) (elfeed-search-set-filter "+emacs +unread")))
    (define-key elfeed-search-mode-map (kbd "v t") (lambda () (interactive) (elfeed-search-set-filter "+tech +unread")))
    (define-key elfeed-search-mode-map (kbd "v c") (lambda () (interactive) (elfeed-search-set-filter "+china +unread")))
    (define-key elfeed-search-mode-map (kbd "v r") (lambda () (interactive) (elfeed-search-set-filter "+reddit +unread"))))
#+end_src
