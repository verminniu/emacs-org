...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  ;; 可选：主题设置（选择你喜欢的主题）
  (use-package magit-theme
    :config
    ;; 示例：使用暗色主题（根据个人喜好选择）
    (setq magit-theme-automatic nil
          magit-theme-default 'magit-theme-solarized-dark))
...
独立归档配置测试方便
#+begin_src emacs-lisp :tangle no
  (defun my/org-get-closed-time ()
    "获取条目的CLOSED时间"
    (org-entry-get (point) "CLOSED"))

  (defun my/org-create-archive-project ()
    "创建归档副本并重组结构，保留原项目不变"
    (interactive)
    (unless (org-entry-is-done-p)
      (user-error "只有已完成的项目可以归档"))
    (save-excursion
      (org-back-to-heading)
      (let* ((orig-heading (org-get-heading))
             (closed-time (my/org-get-closed-time))
             (archive-heading (concat orig-heading " ◀归档▶"))
             (month-heading (when closed-time
                              (format-time-string "%Y/%m"
                               (date-to-time closed-time)))))

        ;; 复制项目树
        (org-copy-subtree nil nil t)
        ...
        ;; 修改归档标题
        (org-edit-headline archive-heading)

        ;; 处理开发计划
        (org-with-point-at (point)
          (when (re-search-forward "^*+ 开发计划" nil t)
            (let ((dev-plan-end (org-end-of-subtree)))
              (when month-heading
                (org-insert-heading-after-current)
                (insert month-heading)
                ;; 移动功能设计条目
                (when (re-search-forward "^*+ TODO 功能设计" dev-plan-end t)
                  (org-cut-subtree)
                  (org-back-to-heading)
                  (org-paste-subtree))
                ;; 删除原开发计划
                (delete-region (point) dev-plan-end)))))))

  ;; 自动触发归档
  (defun my/org-auto-archive-project ()
    (when (and (org-entry-is-done-p)
               (not (org-entry-get (point) "ARCHIVED")))
      (my/org-create-archive-project)
      (org-entry-put (point) "ARCHIVED" "t")))

  (add-hook 'org-after-todo-state-change-hook #'my/org-auto-archive-project)
