...
#+begin_src emacs-lisp :tangle yes
  (use-package awesome-tray
    ...
    ;; 1. 设置你想要显示的信息模块
    ;; 默认包含: (git-branch mode-name location date)
    ;; 可选模块: battery, volume, buffer-read-only 等
    (setq awesome-tray-active-modules '("git-branch" "mode-name" "location" "date"))

    ;; 2. 适配 Nerd Icons (手动指定图标)
    ;; awesome-tray 默认使用文本，你可以通过自定义 face 或简单的字符串注入图标
    (setq awesome-tray-separator "  ") ; 模块之间的间距

    ;; 3. 如果你使用了特定的主题，可以微调颜色以匹配氛围
    ;; (set-face-attribute 'awesome-tray-module-face nil :foreground "cyan" :weight 'bold)
    )
#+end_src
