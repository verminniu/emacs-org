...
独立归档配置测试方便
#+begin_src emacs-lisp :tangle yes
(setq org-archive-location "::* 归档/%s")  ; 动态标题路径模板[4](@ref)

(defun transform-archive-heading ()
  "转换标题层级并插入时间戳"
  (interactive)
  (let* ((orig-heading (nth 4 (org-heading-components)))
         (timestamp (format-time-string "%Y%m%d"))
         (new-heading (concat timestamp "/" orig-heading)))
    (org-edit-headline new-heading)
    (org-demote-subtree)  ; 降级为四级标题
    (save-excursion
      (org-back-to-heading)
      (insert (format "* %s归档\n" (car (split-string orig-heading "/")))))))

;; ====================
;; 自动层级调整
;; ====================
(defun adjust-archive-level ()
  "自动创建时间戳层级结构"
  (let* ((path (split-string (org-extract-archive-heading) "/"))
         (current-level 1))
    (save-excursion
      (goto-char (point-min))
      (dolist (headline path)
        (when (string-match "\$[0-9]\\{8\\}\$" headline)  ; 识别时间戳格式[7](@ref)
          (setq headline (format-time-string "%Y-%m-%d"
                           (parse-time-string headline))))
        (unless (re-search-forward
                (concat "^\\*\\{1," (number-to-string current-level) "\\} " headline) nil t)
          (org-insert-heading)
          (insert headline))
...
(add-hook 'org-archive-hook
  (lambda ()
    (transform-archive-heading)
    (adjust-archive-level)
